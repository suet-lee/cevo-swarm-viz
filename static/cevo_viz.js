...
// @TODO make general for width/height of arena
    ...
    function drawLegend()
    {
        var legCanv = document.querySelector("#legend")
        var legCtx = legCanv.getContext("2d")
        legCanv.width = 200
        legCanv.height = 500
        cpos = $('#arena').offset()
        Otop = cpos['top']
        ...
        label = ["socialize","update","execute"]
        colours = ["#fff","#9e91c2",'#ff6361']

        c1 = 50 // relative to canvas
        c2 = 100
        for (var i=0; i<colours.length; i+=1)
        {
            legCtx.beginPath()
            r = 20
            // legCtx.fillRect(c1,c2,r,r);
            legCtx.fillStyle = colours[i]
            legCtx.strokeStyle = '#000'
            legCtx.lineWidth = 1.5
            legCtx.rect(c1,c2,r,r);
            legCtx.fill()
            legCtx.stroke()
            legCtx.closePath()

            legCtx.beginPath()
            legCtx.font = "14px Arial";
            legCtx.fillStyle = '#2b2b2b';
            legCtx.fillText(label[i], c1+35, c2+15);
            legCtx.closePath()
...
// ############################ Canvas variables
    ...
    var canvasWidth = canvas.width = 500//$('input[name=arena_w]').val()
    var canvasHeight = canvas.height = 500//$('input[name=arena_h]').val()
...
// ############################ Visualize functions
    ...
    function drawRobot(id, x, y) {
        ...
        try {
            ...
            phase = parseInt(G_DATA['r_phase'][ts][id-1])
            if (phase == 0)
            {
                ctx.fillStyle = "#fff";
                ctx.fill()    
