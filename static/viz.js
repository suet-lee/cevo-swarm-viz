        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function(key, val) {
                    ...
                    else if (key == "metrics" && G_CTRL == "rates")
                    {
                        G_DATA["rates"] = val
                    }
                    else if (key == "faults_detected" && G_CTRL == "probaf")
                    {
                        G_DATA["predf"] = val 
                    }
                    else if (key == "faults_type" && G_CTRL == "probaf")
                    {
                        G_DATA["fault_t"] = val 
                    }
                    else if (key == "mitgn_type" && G_CTRL == "probaf")
                    {
                        G_DATA["mitgn_t"] = val 
                    }
                    else if (key == "faults_present" && G_CTRL == "probaf")
                    {
                        G_DATA["fault_p"] = val 
                    }
                    else if (key == "metrics")
...
// ############################ Visualize functions
    ...
    function drawDepositZones()
    {
        ...
        // ctx.fillRect(0, canvasHeight-235, 135, 235);
        // ctx.closePath()

        // ctx.beginPath()
        // ctx.fillStyle = '#f54a3d'
        // ctx.fillRect(0, canvasHeight-185, 85, 185);
        // ctx.closePath()

        // ctx.beginPath()
        // ctx.fillStyle = '#5584fa'
        // ctx.fillRect(canvasWidth-85, 0, 85, 370);
        // ctx.closePath()        
    }
    ...
    function drawRobot(id, x, y, is_faulty=false) {
        ...
        // if (predf != null && predf[id-1] == 1 && (G_CTRL == "trial" || G_CTRL == "probaf"))
        if (predf != null && predf[id-1] == 1 && G_CTRL == "trial")
        {
            ctx.strokeStyle = 'red'
        }
        else
        ...
        if (predf != null && predf[id-1] == 1 && G_CTRL == "probaf" && is_faulty)
        {
            ctx.fillStyle = '#7b03fc';
            ctx.fill()
        }
        else if (is_faulty){ // && (G_CTRL == "collective-transport" || G_CTRL == "probaf" || G_CTRL == "neurov")) {
        ...
        // Label fault type and mitigation applied
        label = "r"+id
        if (G_CTRL == "probaf" && "fault_t" in G_DATA && ts in G_DATA["fault_t"])
        {
            ft = G_DATA["fault_t"][ts][id-1]
            if (ft != 0)
            {
                label += ",f"+ft
            }
        }
        if (G_CTRL == "probaf" && "mitgn_t" in G_DATA && ts in G_DATA["mitgn_t"])
        {
            mt = G_DATA["mitgn_t"][ts][id-1]
            if (mt == 7)
            {
                label += ",B"
            }
